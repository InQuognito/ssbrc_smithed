...
$execute unless items entity @s container.* minecraft:stick[minecraft:custom_data~{item: "goron_locket"}] store success score #damage_success ssbrc.temp run damage @s $(amount) ssbrc:$(type) by @a[predicate=ssbrc:id_match,limit=1]
